...
/*
i am using atom text editor as my text editor to write code beacause its comment color are good.
and i am using "atom-runner" plugin to run the script.
command to run script in window/linux is "alt+r"
*/

//ps- if you dont have a text editor like atom or vscode you can use online tool called as scrimba.
...
javascript provide 7 diffrent type of VARIABLES
1. undefined- variables which can store any data-type.
2. string-sequence of charaxter is called as string for ex-"hello how are you".
...
6. Symbol-they are immutable premetive values.
...
variables declred with var keyword can change there value later in program.

const keyword- it declares global variable which can be use anywhere inside a program
variables declred with "const" keyword cannot change there value later in program.

let keyword- its a local variableit can only be used inside a scope in which it is defined.

*******************************/

var myName = "udit"; //Declaring & intializing  a variable
let myLet = "makkar";
const myConst = 3.14;
...
v1 = 768 //intializing
/*we have to intialize a variable with somee value */
console.log(v1);
...
/*********
Arthematic operation on variables
**************/
...
console.log(v1 - v2); //diffrence;
console.log(v2 * v3); //multiplication
console.log(v1 / v2); //gives questionent (devision)
console.log(v1 % v2); //gives remainder (modulas)
console.log(++v2); // post increment
console.log(--v1); // pre imcrement

/********
assigment with agumented addition and subtraction
********/
//v2+=10; returs a value hence we directly need it to display in log
...
//if you wnat to put "" inside your msg in output use above method
/*************
Escape Sequences
\' single quote
\" double quote
...
/**********
string immutability
once a variable is intialized with the string it cannot
be immutable(edited)
for ex
...
{
  var result = str1 + str2 + str3; //function concatinate the 3 strings
  return result; // return the concatinated string
}
...
ARRAYS
in javascript
array allow is a collection of same or diffrent
data type in a single place
****/
var myArray = ["jhon", 10, 0.12, 'a', true];
//accessing lemnts of the ARRAY
//indexing of array start from 0
console.log(myArray[0]); // jhon
console.log(myArray[1]); // 10
console.log(myArray[2]); // 0.12
console.log(myArray[3]); // a
console.log(myArray[4]); // true
//what will happen when you try to access elemnts of array which are not defined
...
//to access the jhon in ourArray
...
3. shift()
shift removes the first elemnt from the array and return that element
4. unshift()
...
function passFunction(a, b) {
  ...
  /*variable declred outside function with var keyword
    it can be accessed because it is global to the function
  */
  var r = (a - b); //accessing the passed value
  return r //returning the diffrence from the function
}
...
console.log(funcGlobal);
// 121 chnges made to global variable inside the function are persistant
...
{// Destructuring block
  ...
  **/
  ...
  let getTemprature=function(temprature){
    const {tomrrow:dTemprature}=temprature;
    return dTemprature;
  ...
  */
  ...
  let getTemprature2=function(localForcast){
    ...
    const{tomorrow:{max:temp}}=localForcast;
    return temp;
  }
